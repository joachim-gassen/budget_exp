...
df <- rounds %>%
  ...
  select(id, round, pot_slack, claimed_slack, pct_slack_claimed) %>%
  left_join(subjects %>% select(id, treatment))

ggplot(df, aes(x = pot_slack, y = claimed_slack, color = treatment)) +
  geom_jitter() +
  labs(
    x = "Potential budget slack [Taler]",
    y = "Claimed budget slack [Taler]",
    color = "Treatment"
  ) +
  theme_classic() +
...
df %>%
  ...
  summarise(mean_pct_slack_claimed = mean(pct_slack_claimed)) %>%
  ggplot(aes(
    x = round, y = mean_pct_slack_claimed, group = treatment, color = treatment
  )) +
  geom_point() +
  geom_line() +
...
ggplot(outcomes, aes(
  x = pct_slack_claimed, y = treatment,
  ...
  labs(
    title = "Share of budget slack claimed by treatment",
    x = "Share of budget slack claimed",
    y = ""
  ) +
  theme_ridges() +
  theme(
...
ggplot(outcomes %>% filter(rounds == 10), aes(
  x = 10000 + pct_slack_claimed*pot_slack, y = treatment,
  fill = treatment, height = stat(density)
)) +
  geom_density_ridges(
